...
cmake tesseract-$TesseractVersion -B "tesseract_build_${Configuration}" \
  ...
  -DCMAKE_BUILD_TYPE="$Configuration" \
  -DBUILD_SHARED_LIBS=OFF \
  -DLeptonica_DIR="$(pwd)/release/leptonica/$Configuration/lib/cmake/leptonica" \
  -DUSE_SYSTEM_ICU=ON \
...
cp release/leptonica/$Configuration/lib/liblept.lib release/tesseract/$Configuration/lib
